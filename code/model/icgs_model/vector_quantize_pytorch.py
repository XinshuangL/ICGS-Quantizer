...
class EuclideanCodebook(Module):
    def __init__(
        ...
        self.max_update_code_num = max_update_code_num
    ...
    def expire_codes_(self, batch_samples):
        ...
        if expired_codes.sum() > self.max_update_code_num:
            # select the top k codes to update
            print('expired_codes: {}/{}'.format(expired_codes.sum(), self.cluster_size.shape[1]))
            update_indices = (-self.cluster_size).topk(self.max_update_code_num, dim=-1)[1][0, :]
        else:
            update_indices = expired_codes.nonzero()[:, 1]
        
        expired_codes = torch.zeros_like(expired_codes)

        expired_codes[:, update_indices.tolist()] = 1            
        ...
        self.cluster_size[:, update_indices.tolist()] = self.init_cluster_size
...
class VectorQuantize(Module):
    def __init__(
        ...
        codebook_kwargs = dict(
            ...
            max_update_code_num=max_update_code_num
        )
